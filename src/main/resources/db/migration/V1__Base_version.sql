...
CREATE TABLE `wspay_consume` (
  ...
  `trade_no` varchar(100) NOT NULL default '' COMMENT '订单号',
  ...
  `status` tinyint(3) NOT NULL default 0 COMMENT '交易状态: 1: 成功, 2: 失败, 0: 新创建, 3: 超时(失败)',
...
CREATE TABLE `wspay_refund` (
  ...
  `refund_no` varchar(100) NOT NULL default '' COMMENT '退款号',
  ...
  `status` tinyint(3) NOT NULL default 0 COMMENT '退款状态: 1: 成功, 2: 失败, 0: 新创建, 3:超时(成功)',
